...
- **هسته بهینه شده Slipstream Plus**: اکنون با **هسته Rust** و **Tauri** برای نهایت سرعت و پایداری.
- **DNS Checker (اختیاری)**: اجرا کنید و برای ردیف‌هایی که وضعیت **OK** دارند روی دکمه **Use** بزنید تا DNS Resolver همان مقدار تنظیم شود
- **شروع VPN** با دکمه "Connect" (یا Start VPN) و بررسی اینکه وضعیت‌ها "Connected" شوند
...
4. منتظر بمانید تا وضعیت اتصال "Connected" نشان داده شود
...
### macOS: برنامه "Damaged" نشان داده می‌شود
...
- چک کنید پورت‌های `8080` و `5201` اشغال نباشند
- `DNS Resolver` و `Domain` را بررسی کنید
