...
const TrafficCard = ({ label, value, icon: Icon, colorClass }: { label: string, value: string, icon: React.ElementType, colorClass: string }) => (
    ...
    >
        ...
        <div className="flex flex-col">
            ...
            <span className="text-lg font-medium tracking-tight text-foreground">{value}</span>
        </div>
    </motion.div>
);
...
export default function ConnectionPage() {
    ...
    const handleDnsChange = async (val: string) => {
        ...
        if (newResolvers.length > 0) {
            await ipc.invoke('set-resolvers', { resolvers: newResolvers });
            await setSettings({ resolvers: newResolvers });
        }
    };
...
function TimerDisplay() {
    ...
    return <span className="text-xs font-normal opacity-80 tracking-normal">{formatTime(elapsed)}</span>;
}
